      ...
      <div class="admin-items">
          <div class="item-info">
            <div class="item-more-info">
              ...
              <p class="item-detail"> ${books[i].author} | ${
        books[i].publisher
      } | ${published.getFullYear()} </p>
            </div>
      ...
      fetch("/user-admin/categories")
        ...
        .then((data) => {
          ...
          for (let i = 0; i < data.length; i++) {
            if (data[i].name === thisBook.category) {
              categoryList += `<option value="${data[i].name}" selected>${data[i].name}</option>`;
            } else {
              categoryList += `<option value="${data[i].name}">${data[i].name}</option>`;
            }
          }
          setTimeout(() => {
            bookInfoBlock.innerHTML += `
              <div class="book-more-infos">
                <span> <p>제목</p> <input id="titleInput" type="text" value="${
                  thisBook.title
                }"/> </span>
                <span> <p>저자</p> <input id="authorInput" type="text" value="${
                  thisBook.author
                }"/> </span>
                <span> <p>출판사</p> <input id="publisherInput" type="text" value="${
                  thisBook.publisher
                }"/> </span>
                <span> <p>카테고리</p> <select id="categoryInput" name="categories">${categoryList}</select> </span>
                <span> <p>ISBN</p> <input id="isbnInput" type="text" value="${
                  thisBook.isbn
                }"/> </span>
                <span> <p>출판일</p> <input type="date" id="dateInput" name="publication_date" value="${published.getFullYear()}-${(
              published.getMonth() + 1
            )
              .toString()
              .padStart(2, "0")}-${published
              .getDate()
              .toString()
              .padStart(2, "0")}"/> </span>
                <span> <p>가격</p> <input id="priceInput" type="text" value="${
                  thisBook.price
                }"/> </span>
                <span> <p>책소개</p> <textarea id="descriptionInput""/> ${
            ...
            checkBtn.addEventListener("click", () => {
              ...
              const bookPublisher =
                bookInfoBlock.querySelector("#publisherInput");
              const bookCategory =
                bookInfoBlock.querySelector("#categoryInput");
              ...
              const bookDescription =
                bookInfoBlock.querySelector("#descriptionInput");
      ...
      .then((data) => {
        ...
        setTimeout(() => {
          ...
          bookAddBtn.addEventListener("click", () => {
            ...
            const bookDescription =
              bookAddBlock.querySelector("#descriptionInput");
